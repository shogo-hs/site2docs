...
依存関係管理には [uv](https://github.com/astral-sh/uv) を使用します。リポジトリ直下で以下のコマンドを実行してください。
...
```
site2docs/
├── src/site2docs        # パッケージ本体
├── tests                # pytest ベースのテスト
├── output/              # 生成された Markdown / マニフェスト（gitignore 対象）
├── site_backup/         # 入力用アーカイブ（gitignore 対象）
├── pyproject.toml       # ビルド設定と依存
├── uv.lock              # uv ロックファイル
└── .venv/               # ローカル仮想環境（任意）
```
